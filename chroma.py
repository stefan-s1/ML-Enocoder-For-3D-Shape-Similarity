...
def getCollection():
    return chroma_client.get_or_create_collection(name="test-encodings", configuration= {
            "hnsw": {
...
def compare(input1, input2):
    ...
    print(f'Cosine Similarity between {input1} and {input2} is: {dot(a, b/(norm(a)*norm(b)))}')
...
if __name__ == "__main__":
    ...
    p_query.add_argument("-i",   "--input", required=True,
                         help="mesh file to encode")

    p_reset = subs.add_parser("reset_db")
    
    p_encode = subs.add_parser("encode")
    p_encode.add_argument("-d",   "--data", required=True,
                         help="path to directory containing files to index") 

    p_compare = subs.add_parser("compare2")
    p_compare.add_argument("-i1",   "--input1", required=True,
                         help="mesh file to encode")
    p_compare.add_argument("-i2",   "--input2", required=True,
                         help="mesh file to encode")
